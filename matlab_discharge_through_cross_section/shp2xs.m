function    output=shp2xs(swwfile,shp,maxspacing)
%%% ------------------------Descriptions and notes-------------------------
%%% Inputs
%%% swwfile=file name of anuga output
%%% shp=name of shape file -- DO NOT include the .shp extentsion  
%%% maxspacing=maximum distance between points; note can set to 0 if you
%%% don't want the code to densify the points
...
%% Start converting shapefile to struct
s=m_shaperead(shp); %%% see function below its not one I wrote
ind=length(s.ncst); %%% Get number of cross sections
for i =1:ind
    xy=s.ncst{i};  %%% Extract xy data from each line
    ...
    if maxspacing>0
        ...
        for j=1:length(dX)
            xt=linspace(xy(j,1),xy(j+1,1),ns(j));%%linearly densify x direction
            yt=linspace(xy(j,2),xy(j+1,2),ns(j));%%linearly densify y direction
    ...
    [in,on]=inpolygon(xy(:,1),xy(:,2),bx,by);
    in=logical(in+on);%% comine inside and edge of domain
    xy=xy(in,:); %%keep only points in domain

    
    %% Write data to output
    varname=['xs',num2str(i)];
    output.(varname)=xy;
end



%%% The below function comes from https://www.eoas.ubc.ca/~rich/mapug.html
%%% an open source mapping toolbox for matlab
